...
class ConnectionManager:
    ...
    async def _receive_socket(self) -> bytes:
        ...
        try:
            buffer = deque()
            while True:
                # Find start of frame (0x02)
                initial_data = await self.reader.read(1)
                if not initial_data:
                    return b''
                
                if initial_data[0] != 0x02:
                    continue
                
                buffer.append(initial_data[0])
                
                # Read header and length bytes
                header_info = await recv_exactly(2)
                buffer.extend(header_info)
                
                # Framing logic based on Header and Length byte
                if len(buffer) < 3:
                    continue # Should not happen if recv_exactly worked, but for safety
                
                header = buffer[1]
                length = buffer[2]
                
                # Determine actual packet size
                if header in [0x31, 0x41, 0x61] and length not in [13, 19, 20, 30]:
                    # These are 10-byte fixed length packets where length byte is timestamp/etc.
                    pkt_size = 10
                elif length > 0:
                    pkt_size = length
                else:
                ...
                remaining = await recv_exactly(pkt_size - len(buffer))
                buffer.extend(remaining)
                
                packet_data = bytes(buffer)
                # Parse basic info for logging
                if len(packet_data) >= 4:
                    header_val = packet_data[1]
                    command_val = packet_data[3]
                    LOGGER.debug(f"Received packet: length={len(packet_data)}, room_byte={hex(header_val)}, command_byte={hex(command_val)}, hex={packet_data.hex()}")
...
class BestinHub:
    ...
    async def determine_gateway_mode(self) -> None:
        ...
        try:
            ...
            for chunk in chunk_storage:
                ...
                if (command_byte in [0x91, 0xB2, 0x81, 0x82]):
                    if chunk_length in [20, 22]:
            ...
            if aio_data:
                ...
                LOGGER.info(f"AIO mode set with data: {aio_data}")
            elif gen2_data:
                self.gateway_mode = ("Gen2", gen2_data)
                LOGGER.info(f"Gen2 mode set with data: {gen2_data}")
            else:
                self.gateway_mode = ("General", None)
                LOGGER.info("No smart packets found, defaulting to General mode")
        except Exception as e:
    ...
    async def async_initialize_serial(self) -> None:
        ...
        try:
            # Load saved mode from config entry and skip redetection if exists
            saved_mode = self.entry.data.get("gateway_mode")
            if saved_mode:
                self.gateway_mode = tuple(saved_mode)
                LOGGER.info(f"Using PERSISTENT gateway mode: {self.gateway_mode[0]}")
            
            if self.gateway_mode is None:
                LOGGER.info("No saved gateway mode, starting detection...")
