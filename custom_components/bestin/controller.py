...
# AsyncQueue removed - using standard asyncio.Queue
...
class BestinController:
    ...
    def make_light_packet(
        ...
        if aio_gateway:
            ...
            packet[5] = 0x01 if value or sub_type in ["brightness", "color_temp"] else 0x00
            packet[6] = 10 if pos_id == 4 else 1 << pos_id
            
            # Smart light control for AIO
            if sub_type == "brightness":
                packet[5] = 0x01 # Ensure on
                packet[8] = value # Brightness 1-10
            elif sub_type == "color_temp":
                packet[5] = 0x01 # Ensure on
                packet[7] = value # ColorTemp 1-7
    ...
    @callback
    async def enqueue_command(self, device_id: str, value: Any = None, **kwargs: dict | None):
    ...
    def parse_thermostat(self, packet: bytearray) -> tuple[int | None, dict | None]:
    ...
    def parse_gas(self, packet: bytearray) -> tuple[int | None, bool | None]:
    ...
    def parse_doorlock(self, packet: bytearray) -> tuple[int | None, bool | None]:
    ...
    def parse_fan(self, packet: bytearray) -> tuple[int | None, dict | None]:
    ...
    def parse_state_general(self, packet: bytearray) -> tuple[dict, int]:
        ...
        room_id = packet[5] & 0x0F if len(packet) > 5 else 0
        if room_id == 1:
            iterations = (4, 3)
        else:
            iterations = (2, 2)
        ...
        for i in range(iterations[0]):
            if len(packet) <= 6: break
            light_state = bool(packet[6] & (0x01 << i))
            dc_value = int.from_bytes(packet[12:14], 'big') / 10.0 if len(packet) >= 14 else 0.0
        ...
        for i in range(iterations[1]): 
            ...
            if i < 2:
                ...
                if len(packet) >= idx2:
                    ...
                    state_general["outlet"][f"cutvalue_{str(i)}"] = 0.0
    ...
    def parse_state_aio(self, packet: bytearray) -> tuple[int, dict]:
        ...
        room_id = packet[1] & 0x0F if len(packet) > 1 else 0
        
        # Room status for light
        light_count = packet[5] if len(packet) > 5 else 0
        for i in range(min(light_count, 10)):
            light_state = bool(packet[6] & (1 << i))
            # AIO Smart light data
            state_aio["light"][str(i)] = {
                "state": light_state,
                "brightness": packet[10] if len(packet) > 10 else None,
                "color_temp": packet[11] if len(packet) > 11 else None,
            }
    ...
    def parse_energy(self, packet: bytearray) -> dict:
        ...
        for element in elements:
            try:
                raw_total = packet[ENERGY_BYTE_RANGE[element][element_offset]].hex()
                total_value = float(raw_total) if raw_total else 0.0
            except ValueError:
                total_value = 0.0

            try:
                raw_real = packet[index:index + 2].hex()
                realtime_value = int(raw_real) if raw_real else 0
            except ValueError:
                realtime_value = 0
    ...
    def validate_response(self, packet: bytes, queue: dict):
        ...
        if not queue.get("command_packet"):
            return
    ...
    async def send_packet_queue(self, queue: dict):
        ...
        LOGGER.info(
            ...
            queue["value"], queue["device_type"], queue["command_packet"].hex(), queue["send_retry"] + 1
        )
        queue["send_retry"] += 1
        self.timestamp = (self.timestamp + 1) & 0xFF
    ...
    def handle_device_packet(self, packet: bytes):
        ...
        valid_commands = [0x11, 0x12, 0x19, 0x21, 0x81, 0x82, 0x91, 0x92, 0xA1, 0xB1, 0xB2]
        is_aio_header = 0x51 <= header <= 0x5F
        if packet_len != 10 and (header in [0x28, 0x31, 0x61, 0x17, 0xD1] or is_aio_header or command in valid_commands):
            try:
                if header == 0x28:
                    room_id, device_state = self.parse_thermostat(packet)
                    if room_id is not None and device_state is not None:
                    ...
                    # Packet length-based parser selection for robustness
                    if packet_len in [19, 30]:
                        room_id, device_state = self.parse_state_general(packet)
                    elif packet_len in [20, 22]:
                        if self.gateway_type == "General":
                            LOGGER.info("Detected AIO-style packet in General mode. Switching gateway_type to AIO.")
                            self.gateway_type = "AIO"
                            self.hub.gateway_mode = ("AIO", None)
                        room_id, device_state = self.parse_state_aio(packet)
                    elif packet_len in [27, 41, 55, 59, 72, 98, 150]:
                        room_id, device_state = self.parse_state_gen2(packet)
                    else:
                        # Fallback to detected gateway mode parser
                        parser_name = f"parse_state_{self.gateway_type}".lower()
                        room_id, device_state = getattr(self, parser_name)(packet)
                    
                    if room_id is not None and device_state:
                         for device, state in device_state.items():
                            device_id = f"{device}_{room_id}"
                            self.set_device(device_id, state, is_sub=True)
            except Exception as ex:
                LOGGER.error(f"Error processing device packet: {ex} (Header: {hex(header) if header is not None else 'None'}, Len: {packet_len})", exc_info=True)
    ...
    async def handle_packet_queue(self, queue: dict):
        """Handle a packet from the queue with retry logic and ACK waiting."""
        self._current_command = queue
        max_retry = self.entry.options.get("max_send_retry", DEFAULT_MAX_SEND_RETRY)
        
        try:
            while queue["send_retry"] < max_retry:
                await self.send_packet_queue(queue)
                try:
                    # Wait for ACK with timeout
                    await asyncio.wait_for(queue["ack_event"].wait(), timeout=1.5)
                    LOGGER.info(
                        "%s: Success (ACK: %s, Attempts: %d)",
                        queue["device_type"], queue["acknowledgment"].hex(), queue["send_retry"]
                    )
                    return True
                except asyncio.TimeoutError:
                    LOGGER.debug(
                        "%s: ACK timeout, retrying (%d/%d)",
                        queue["device_type"], queue["send_retry"], max_retry
                    )
                    continue
        ...
        while True:
            ...
            try:
                ...
                if not received_data:
                    # Some connections return None or empty bytes on timeout/idle
                ...
                if checksum_valid:
                    ...
                    if self._current_command:
                        self.validate_response(received_data, self._current_command)
            except Exception as ex:
    ...
    def log_packet_viewer(self, data: bytes, checksum_valid: bool):
        ...
        # Forced enabled and set to INFO for user debugging
        checksum_status = "valid" if checksum_valid else "invalid"
        formatted_data = ' '.join(f'{byte:02X}' for byte in data)
        LOGGER.info("Packet viewer - Data: %s | Checksum: %s", formatted_data, checksum_status)
